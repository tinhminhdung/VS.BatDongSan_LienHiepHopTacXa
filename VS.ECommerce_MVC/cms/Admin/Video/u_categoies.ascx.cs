    ...
    public partial class u_categoies : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!base.IsPostBack)
            {
                ...
                #endregion
                if (!Commond.Setting("PageCateVideo").Equals(""))
                {
                    ddlPage.SelectedValue = Commond.Setting("PageCateVideo");
                }
                this.LoadItems();
            }
        }
        ...
        public void LoadItems()
        {
            ...
            List<Entity.Menu> dt = SMenu.CATEGORY_PHANTRANG(More.VD, (pages - 1), ref Tongsobanghi, Tongsotrang);
           // if (dt.Count >= 1)
            {
            ...
            ltpage.Text = Commond.PhantrangAdmin("/admin.aspx?u=Video&su=Video", Tongsobanghi, pages);
        ...
        protected void txtTennhom_TextChanged(object sender, EventArgs e)
        {
            ...
            if (Nhom.Text.Length > 0)
            {
                ...
                if (item.Count > 0)
                {
                    ...
                    {
                        if (MoreAll.AddURL.SeoURL(item[0].TangName) != MoreAll.AddURL.SeoURL(Nhom.Text))
                        {
                            var hasTagName = db.ModulControls.Where(s => s.TangName == MoreAll.AddURL.SeoURL(Nhom.Text)).FirstOrDefault();
                            TagName = hasTagName != null ? MoreAll.AddURL.SeoURL(Nhom.Text) + "-" + obk.ID : MoreAll.AddURL.SeoURL(Nhom.Text);
                        }
                        else
                        {
                            TagName = item[0].TangName;
                        }
                        Menu obl = db.Menus.SingleOrDefault(p => p.ID == int.Parse(b.Value));
                        obl.Name = Nhom.Text;
        ...
        protected void ddlCaps_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string sss = More.Level_ID(ddlCap.SelectedValue, More.VD);
        ...
        {
            MoreAll.MoreAll.Update_setting("PageCateVideo", ddlPage.SelectedValue);
        ...
        protected void bntcapnhat_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < rp_pagelist.Items.Count; i++)
            {
                ...
                {
                    SMenu.Name_Text("UPDATE [Menu] SET Orders=" + txtOrders.Text + " WHERE ID=" + id.Value + " and capp='" + More.VD + "'");
