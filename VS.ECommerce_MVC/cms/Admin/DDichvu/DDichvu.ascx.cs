...
namespace VS.ECommerce_MVC.cms.Admin.DDichvu
{
                ...
                if (str2 != null)
                {
                    if (!(str2 == "update"))
                    {
                        if (str2 == "insert")
                        {
                            ...
                            int cong = 0;
                            string TangName = "";
                            List<Entity.Dichvu> curItem = SDichvu.Name_Text("SELECT top 1 * FROM Dichvu order by ID desc");
                            int tong = int.Parse(curItem[0].ID.ToString()); cong = tong + 1; var hasTagName = db.Dichvus.Where(s => s.TangName == MoreAll.AddURL.SeoURL(txtname.Text)).FirstOrDefault(); TangName = hasTagName != null ? MoreAll.AddURL.SeoURL(txtname.Text) + "-" + cong : MoreAll.AddURL.SeoURL(txtname.Text);
                            #endregion
                    ...
                    else
                    {
                        ...
                        if (item.Count > 0)
                        {
                            List<Dichvu> list = (from p in db.Dichvus where p.TangName == item[0].TangName orderby p.ID descending select p).ToList();
                            if (list.Count > 2)
                            {
                                var hasTagName = db.Dichvus.Where(s => s.TangName == MoreAll.AddURL.SeoURL(txtname.Text)).FirstOrDefault(); TagName = hasTagName != null ? MoreAll.AddURL.SeoURL(txtname.Text) + "-" + item[0].ID : MoreAll.AddURL.SeoURL(txtname.Text);
                            }
                            else
                            {
                                if (MoreAll.AddURL.SeoURL(item[0].Title) != MoreAll.AddURL.SeoURL(txtname.Text)) { var hasTagName = db.Dichvus.Where(s => s.TangName == MoreAll.AddURL.SeoURL(txtname.Text)).FirstOrDefault(); TagName = hasTagName != null ? MoreAll.AddURL.SeoURL(txtname.Text) + "-" + item[0].ID : MoreAll.AddURL.SeoURL(txtname.Text); } else { TagName = item[0].TangName; }
                            }
                        }
                        #endregion
            ...
            switch (e.CommandName)
            {
                ...
                case "Delete":
                    ...
                    if (dt.Count > 0)
                    {
                        ...
                        try
                        {
                            SDichvu.Name_Text("DELETE FROM Dichvu WHERE TangName=N'" + dt[0].TangName + "'");
                        }
                        catch (Exception)
                        { }
                    }
                ...
                for (int i = 0; i < rpitems.Items.Count; i++)
                {
                    ...
                    if (chk.Checked)
                    {
                        ...
                        for (int j = 0; j < dlt.Count; j++)
                        {
                            ...
                            try
                            {
                                SDichvu.Name_Text("DELETE FROM Dichvu WHERE TangName=N'" + dlt[j].TangName + "'");
                            }
                            catch (Exception)
                            { }
                        }
