...
namespace Framework
{
    public class FGioithieu
    {
        ...
        {
            SqlConnection conn = Database.Connection();
            SqlCommand comm = new SqlCommand("select * from Gioithieu where  Status= 1 and lang=@lang  order by Create_Date asc", conn);
            comm.CommandType = CommandType.Text;
            comm.Parameters.Add(new SqlParameter("@lang", lang));
            try
            {
        ...
        {
            string sql = @"select * from Gioithieu  where (Title like '%'+@keyword1+'%'  or Brief like '%'+@keyword2+'%'  or Contents like '%'+@keyword3+'%'  or search like '%'+@keyword4+'%') and lang=@Lang and Status=1   order by Create_Date desc";
        ...
        {
            string sql = @"select  top " + top.ToString() + " * from Gioithieu where Create_Date < (select Create_Date from Gioithieu where id=@id) and lang=@lang   order by Create_Date desc";
            SqlConnection conn = Database.Connection();
            SqlCommand comm = new SqlCommand(sql, conn);
            comm.CommandType = CommandType.Text;
            comm.Parameters.Add(new SqlParameter("@id", id));
